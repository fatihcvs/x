...
3. **Billing**'den ödeme/kredi ekle (anahtar kredisiz çalışmaz). Tweet botu az
   tüketir; yine de Opus modeli ucuz modellerden pahalıdır (aşağıdaki maliyet notu).
...
- `model` — içeriği yazan Claude modeli. Varsayılan `claude-opus-4-8` (en doğal);
  daha dengeli/ucuz için `claude-sonnet-4-6`, en ucuz için `claude-haiku-4-5`.
...
**Tahmini maliyet:** `claude-opus-4-8` ile tweet başına birkaç sent (günde birkaç
tweet + cilalama ≈ aylık ~$5-15). Düşürmek istersen `claude-sonnet-4-6` ya da
`claude-haiku-4-5` seç ve/veya `refineTweets: false` yap. X tarafı günde 4 tweet
≈ ayda ~$2-4.
